/**
 * Cloudflare Worker for Media Tracker API
 * Serverless backend using D1 SQLite database
 */
  ...
  try {
    ...
    if (method === 'GET' && path === '/api/stats') {
      ...
      results.forEach(row => {
        ...
        stats.thisMonth += row.thisMonth;
        stats.thisYear += row.thisYear;
    ...
    // Delete entry
    if (method === 'DELETE' && path.startsWith('/api/admin/entries/')) {
      const id = path.split('/').pop();
    ...
    // Clear all data
    if (method === 'DELETE' && path === '/api/admin/clear-all') {
